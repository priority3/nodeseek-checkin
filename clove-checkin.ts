...
const BASE_URL = "https://clove.cc.cd";
...
function buildCookieHeader(session: string, userId: string): string {
  ...
  return `session=${sessionValue}; new-api-user=${userId}`;
}
...
async function checkin(): Promise<void> {
  const userId = (process.env.CLOVE_USER ?? "").trim();
  if (!userId) {
    console.error("ERROR: CLOVE_USER is not set.");
    process.exit(1);
  }

  const session = (process.env.CLOVE_SESSION ?? "").trim();
  if (!session) {
    console.error("ERROR: CLOVE_SESSION is not set.");
    process.exit(1);
  }

  const cookie = buildCookieHeader(session, userId);

  console.log("=== Clove Check-in ===");
  ...
  if (!checkinRes.ok) {
    title = `Clove签到失败: HTTP ${checkinRes.status}`;
    ...
    try {
      ...
      if (typeof data?.success === "boolean" && !data.success) {
        ...
        title = `Clove签到: ${checkinMessage || "失败"}`;
        ...
        const alreadyCheckedIn =
          checkinMessage.includes("已签到") || checkinMessage.includes("already");
        title = `Clove签到: ${checkinMessage || `code=${data.code}`}`;
...
checkin().catch(async (err: Error) => {
  ...
  await notify(`Clove签到异常: ${err.message}`, err.stack ?? "");
  process.exit(1);
});
