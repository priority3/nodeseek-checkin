...
const SITE_NAME = "DeepFlood";
const COOKIE_ENV_NAME = "DF_COOKIE";
const CHECKIN_URL = "https://www.deepflood.com/api/attendance?random=true";
const SITE_URL = "https://www.deepflood.com/board";
...
const DEFAULT_USER_DATA_DIR = path.resolve(__dirname, ".profiles", "deepflood-browser-profile");
...
function parseCookies(cookieStr: string): CookieParam[] {
  return cookieStr
    ...
    .map((c) => {
      ...
      return { name, value, domain: ".deepflood.com", path: "/" };
    })
...
function getBrowserProxyServer(): string {
  const explicitProxy = (process.env.DF_PROXY_SERVER ?? "").trim();
...
function getManualBootstrapHint(userDataDir: string): string {
  return (
    `Run \`npm run checkin:deepflood:init\` once to complete Cloudflare/login in a visible browser, ` +
...
async function waitForReusableSessionCookie(
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    if (!loginHintShown) {
      console.log(
        `No authenticated ${SITE_NAME} session is present yet. If the login page is open, sign in manually in the visible browser window; this script will keep polling.`
      );
      loginHintShown = true;
    }
...
async function applyEnvCookiesToPage(page: Page, cookies: CookieParam[]): Promise<void> {
  ...
  console.log(`Applying ${COOKIE_ENV_NAME} to the browser session...`);
...
async function ensureSiteSession(
  ...
  const maxWaitMs = allowManualMode
    ? parsePositiveInt(process.env.DF_BOOTSTRAP_WAIT_MS, BOOTSTRAP_MAX_WAIT_MS)
    : CF_MAX_WAIT_MS;
  let usedEnvCookies = false;

  console.log(`Navigating to ${SITE_NAME}...`);
  ...
  if (shouldRefreshProfileFromEnv(pageCookies, cookies)) {
    console.log(
      `The persistent browser profile does not match the provided ${COOKIE_ENV_NAME}. Refreshing the profile from ${COOKIE_ENV_NAME}...`
    );
    ...
    console.log(
      `No reusable ${SITE_NAME} session was found in the persistent browser profile. Seeding the profile from ${COOKIE_ENV_NAME}...`
    );
    await applyEnvCookiesToPage(page, cookies);
    usedEnvCookies = true;
    await page.reload({
  ...
  if (!hasSession) {
    throw new Error(
      `No reusable ${SITE_NAME} session is available in ${userDataDir}. ${getManualBootstrapHint(
        userDataDir
      )}`
    );
  }
...
async function runCheckinAttempt(cookies: CookieParam[]): Promise<CheckinResult> {
  const runHeadful = isTruthyEnv(process.env.DF_HEADFUL);
  const allowManualMode = runHeadful && isTruthyEnv(process.env.DF_MANUAL_CF);
  const proxyServer = getBrowserProxyServer();
  const userDataDir = (process.env.DF_USER_DATA_DIR ?? "").trim() || DEFAULT_USER_DATA_DIR;
  const launchArgs = [
    "--no-sandbox",
  ...
  try {
    ...
    await ensureSiteSession(page, cookies, userDataDir, allowManualMode);

    let result = await sendCheckinRequest(page);

    if (looksLikeAuthFailure(result.status, result.body) && cookies.length > 0) {
      console.log(
        `The persisted browser session looks expired. Refreshing the profile from ${COOKIE_ENV_NAME} and retrying once...`
      );
      await applyEnvCookiesToPage(page, cookies);
      await page.reload({
        waitUntil: "domcontentloaded",
    ...
    if (looksLikeAuthFailure(result.status, result.body)) {
      throw new Error(
        `${SITE_NAME} session is no longer authenticated. Refresh ${COOKIE_ENV_NAME} or ${getManualBootstrapHint(
          userDataDir
        )}`
      );
    }
    ...
    try {
      ...
      if (data.success) {
        title = `[deepflood-checkin] ${SITE_NAME}签到成功 +${data.gain}鸡腿 (总计${data.current})`;
        body = data.message ?? result.body;
        console.log("Check-in succeeded!");
      } else if (data.message) {
        title = `[deepflood-checkin] ${SITE_NAME}签到: ${data.message}`;
        body = result.body;
        console.log(`Check-in result: ${data.message}`);
      } else {
        title = `[deepflood-checkin] ${SITE_NAME}签到失败: 未知响应`;
        body = result.body;
        failed = true;
      }
    } catch {
      title = `[deepflood-checkin] ${SITE_NAME}签到失败: HTTP ${result.status}`;
      body = result.body;
      failed = true;
    }
...
async function bootstrapSession(cookies: CookieParam[]): Promise<void> {
  ...
  try {
    ...
    await page.setViewport({ width: 1365, height: 900 });
    await page.setBypassCSP(true);
    await ensureSiteSession(page, cookies, userDataDir, true);
...
async function checkin(): Promise<void> {
  console.log(`=== ${SITE_NAME} Check-in ===`);
  console.log(`Time: ${new Date().toISOString()}`);

  const cookieStr = process.env.DF_COOKIE ?? "";
  const cookies = cookieStr ? parseCookies(cookieStr) : [];
  console.log(`Parsed ${cookies.length} cookies`);

  if (isTruthyEnv(process.env.DF_BOOTSTRAP)) {
    console.log(
  ...
  throw lastError ?? new Error(`${SITE_NAME} check-in failed for an unknown reason.`);
}
...
checkin().catch(async (err: Error) => {
  ...
  await notify(`[deepflood-checkin] ${SITE_NAME}签到异常: ${err.message}`, err.stack ?? "");
  process.exit(1);
});
