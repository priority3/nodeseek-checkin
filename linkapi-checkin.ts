...
const BASE_URL = "https://linkapi.ai";
const CHECKIN_URL = `${BASE_URL}/api/user/checkin`;
const REFERER_URL = `${BASE_URL}/profile`;
...
// Reason: linkapi.ai sits behind Cloudflare, so the full cookie string
// (including cf_clearance and session) must be forwarded verbatim.
function buildHeaders(userId: string, cookie: string): Record<string, string> {
  return {
...
async function checkin(): Promise<void> {
  const userId = (process.env.LINKAPI_USER ?? "").trim();
  if (!userId) {
    console.error("ERROR: LINKAPI_USER is not set.");
    process.exit(1);
  }

  const cookie = (process.env.LINKAPI_COOKIE ?? "").trim();
  if (!cookie) {
    console.error("ERROR: LINKAPI_COOKIE is not set.");
    process.exit(1);
  }

  console.log("=== LinkAPI Check-in ===");
  console.log(`Time: ${new Date().toISOString()}`);
  console.log(`User: ${userId}`);

  const checkinRes = await fetch(CHECKIN_URL, {
    method: "POST",
    headers: buildHeaders(userId, cookie),
  });
  ...
  if (!checkinRes.ok) {
    title = `LinkAPI签到失败: HTTP ${checkinRes.status}`;
...
checkin().catch(async (err: Error) => {
  ...
  await notify(`LinkAPI签到异常: ${err.message}`, err.stack ?? "");
  process.exit(1);
});
